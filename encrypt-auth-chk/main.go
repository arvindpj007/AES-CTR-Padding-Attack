...
func getChecksum(plaintext string) string {
	...
	var sum = plaintext[0:2]
	for i := 2; i < len(plaintext); i += 2 {

		sum = getSum(sum, plaintext[i:i+2], true)
	}
...
func aesctrDecrypt(key, ciphertext string) string {
	...
	checksumActual = plaintext[0:2]
	plaintextWithoutChecksum = plaintext[2:n]
...
func main() {
	...
	case "encrypt":
		...
		// Step 2. M′ = T||M
		messageAndChecksum = checksumValue + plaintext
