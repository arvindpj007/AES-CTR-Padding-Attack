...
func getDecryptTestResult() string {
	...
	cmd := exec.Command("./decrypt-crc-test", "-i", "check.txt")
...
func partitionCipherText(cipherText string) (string, []string, int) {
	...
	numberOfPartitions := int((size - 32) / 32)
...
func oracleAttack(iv, cipherText string) string {
	...
	for i := 4294967295; i >= 0; i-- {
		...
		if len(lastByte) < 8 {
			lastByte = duplicateZeros1(8-len(lastByte)) + lastByte
		}

		// fmt.Println("iteration: ", lastByte, i)
		ciphertextPart = lastByte
	...
	for j := 0; j < nCipherText-12; j++ {

		zeros := duplicateZeros(j + 1)
		sum := crc32.ChecksumIEEE(hexToBytes(zeros))
		nextChecksum := fmt.Sprintf("%x", sum)

		//find XOR of nextChecksum[0:6] and nextByte[len(nextByte)-6:len(nextByte)]

		input1 := duplicateZeros(13) + nextChecksum[0:6]
		nNextByte := len(nextByte)
		input2 := duplicateZeros(13) + nextByte[nNextByte-6:nNextByte]
		output1 := hexXOR(input1, input2, 32)
		middleBytes := output1[26:32]

		firstByte := string(nextByte[0 : 2+2*j])
		...
		for i := 0; i < 255; i++ {
			...
			// fmt.Println("iteration: ", lastByte, i)
			ciphertextPart = firstByte + middleBytes + lastByte
			// fmt.Println(ciphertextPart)
			input = iv + ciphertextPart
			setCheckFile(hexToBin(input))
			result = getDecryptTestResult()
		...
		input1 = duplicateZeros(15) + nextChecksum[6:8]
		input2 = duplicateZeros(15) + lastByte
		output1 = hexXOR(input1, input2, 32)
		nextByte = nextByte + output1[30:32]
	}

	ivDeciphered = nextByte

	// fmt.Println("IV deciphered: ", ivDeciphered, size)

	if nCipherText < 32 {

		ivDeciphered = duplicateZeros(16-size) + ivDeciphered
...
func main() {
	...
	// fmt.Println("IV: ", iv)
	// fmt.Println("parted cipher texts: ", partedCipherText, len(partedCipherText))
	...
	plainText = plaintextAndChecksum[0 : nPlaintextAndChecksum-8]
